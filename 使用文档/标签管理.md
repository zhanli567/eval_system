...
| **配置项** | **说明**                                                     |
| ---------- | ------------------------------------------------------------ |
| 标签名称   | 必填，1-50字。用于标识和区分不同的标签，建议使用清晰易懂的名称。 |
| 描述       | 非必填，0-200字。描述标签的用途、评判标准或使用说明。        |
| 类型       | 选择标签类型，支持分类、布尔值、数字、文本四种类型。详见下文"标签类型"。 |
...
| **配置项** | **说明**                                               |
| ---------- | ------------------------------------------------------ |
| 筛选项     | 支持自定义筛选选项，最多可添加20个选项。               |
| 标注方式   | 标注时以下拉多选的方式进行选择，可选择一个或多个选项。 |
| 筛选条件   | 支持的筛选条件：属于、不属于。                         |
...
| **配置项** | **说明**                                                     |
| ---------- | ------------------------------------------------------------ |
| 数据类型   | 支持Double类型数值输入，可以输入整数或小数。                 |
| 标注方式   | 标注时通过数字输入框填写具体数值。                           |
| 筛选条件   | 支持的筛选条件：等于、不等于、大于、大于等于、小于、小于等于。 |
...
| **配置项** | **说明**                                       |
| ---------- | ---------------------------------------------- |
| 数据类型   | 支持String类型文本输入，可以输入任意文本内容。 |
| 标注方式   | 标注时通过文本输入框填写具体内容。             |
| 筛选条件   | 支持的筛选条件：包含、不包含。                 |
...
| **模式**     | **说明**                                                     |
| ------------ | ------------------------------------------------------------ |
| **普通模式** | 页面为平铺展示，字段横向排列。点击操作列的**标注**可查看单条数据的完整信息，逐条进行标注。 |
| **快速标注** | 自定义标签为可编辑状态。分类标签和布尔值标签显示为下拉选择，数字和文本标签显示为输入框。选择或输入数据后会立即保存。 |
...
-   **根据标签进行数据筛选**：点击**过滤器****添加筛选条件**后，然后点击**应用**即可筛选指定内容。![image](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=)
...
1.  在指定应用右侧点击**查看详情**，点击**数据标注**。![image](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=)
    
2.  在弹出的侧边栏中，选择要添加的标签。点击**新建标签**可跳转到标签管理页面创建新标签。![image](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=)
...
在应用观测页面的过滤器中，用户手动添加过的标签会自动出现在筛选项中。![image](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=)