...
export const tagTypeOptions: { label: string; value: TagType }[] = [
...
const booleanOptions: TagOption[] = [
...
export function useTagManagement() {
  ...
  async function openEditDialog(row: TagSummary) {
  ...
  function fillForm(detail: TagDetail) {
  ...
  function addCategoryOption(group: TagOptionGroup) {
    if (group === 'pass') {
  ...
  function removeCategoryOption(group: TagOptionGroup, index: number) {
  ...
  function buildPayload() {
    const options: TagOption[] =
      tagForm.tagType === 'category'
        ? [
            ...cleanOptions(tagForm.passOptions).map((optionName) => ({ optionName, optionGroup: 'pass' as const })),
            ...cleanOptions(tagForm.failOptions).map((optionName) => ({ optionName, optionGroup: 'fail' as const }))
          ]
        : []
    return {
  ...
  function getTagTypeLabel(value?: string) {
